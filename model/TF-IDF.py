...
class SupervisedTopicLabeler:
    ...
    def __init__(self):
        ...
        # Keyword system (fallback for low-confidence cases)
    ...
    def train(self, manual_df, keyword_df=None, keyword_confidence_threshold=0.5,
              ...
              do_cross_validation=True, cv_folds=5, cv_n_jobs=None):
        """
        ...
        if do_cross_validation:
            ...
            cv_scores = cross_val_score(self.model, X_combined, y_combined, cv=cv_folds,
                                        scoring='accuracy', n_jobs=cv_n_jobs)
            prec_scores = cross_val_score(self.model, X_combined, y_combined, cv=cv_folds,
                                          scoring='precision_weighted', n_jobs=cv_n_jobs)
            print(f"   CV ({cv_folds}-fold) Accuracy: {cv_scores.mean():.3f} (+/- {cv_scores.std():.3f})")
            print(f"   CV ({cv_folds}-fold) Precision (weighted): {prec_scores.mean():.3f} (+/- {prec_scores.std():.3f})")
    ...
    def predict(self, df, text_cols=['title', 'description'], 
                confidence_threshold=0.4, use_keyword_fallback=True):
        """
...
if __name__ == "__main__":
    # Paths
    DATA_DIR = Path(__file__).parent.parent / "data_africa"
    
    MANUAL_LABELED_FILE = DATA_DIR / "manual_training_set.csv"
    ...
    # Expected columns in current environment: text, label (and optionally language_code)
    # Ensure compatibility with training that expects ['title','description'] text inputs
    if {'text', 'label'}.issubset(manual_df.columns):
        manual_df = manual_df.copy()
        manual_df['title'] = ''
        manual_df['description'] = manual_df['text']
    # If already has title/description/topic_label, it's fine as-is
    ...
    labeler.train(
        ...
        manual_label_col=('label' if 'label' in manual_df.columns else 'topic_label'),  # Manual ground truth column
        keyword_label_col='predicted_label_id',  # From old code
        keyword_conf_col='prediction_confidence',  # From old code
        do_cross_validation=False,  # Fast mode by default to avoid long runs
        cv_folds=3,
        cv_n_jobs=-1
    )
    ...
    results_df = labeler.predict(
        unlabeled_df,
        text_cols=['title', 'description'],
        confidence_threshold=0.5,      # Tuned threshold
...
📊 CURRENT STATUS:
    - Training data (manual rows): {len(manual_df)}
    - Articles labeled this run: {len(results_df)}
